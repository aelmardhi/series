...
Store.prototype.set = function(name, path){
    ...
    const shortestPath = shortest === MAIN ? path : posixPathModule.format(pathModule.parse( pathModule.relative(shortest, path)));
...
Store.prototype.getClosestConfig = function(path){
    ...
    let length = pathModule.relative(shortest, path).split(pathModule.sep).length
    const configKeys = Object.keys( this.config)
    for (let k of configKeys ){
        if(k === MAIN)
            k = MAIN_CONFIG_PATH;
        const p = pathModule.relative(path, k).split(pathModule.sep);
...
Store.prototype.list = async function () {
    let list = '';
...
Store.prototype.reset = async function (name) {
    const key = this.findKey(name);
    this.config[key][name].current =  undefined
    this.saveConfig(key);
}

Store.prototype.delete = async function (name) {
    const key = this.findKey(name);
    this.config[key][name] =  undefined
    this.saveConfig(key);
}
