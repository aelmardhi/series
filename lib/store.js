...
const MAIN = 'main';
const MAIN_CONFIG_PATH = posixPathModule.join(__dirname.replaceAll('\\','/'),'..',FILE_NAME);
const numRegExp = /\d+/g;
...
Store.prototype.set = function(name, path){
    path = posixPathModule.resolve(path);
    const shortest = this.getClosestConfig(path);
    this.config[shortest][name] =  {
        path: posixPathModule.join( posixPathModule.relative(shortest, path)),
    }
...
Store.prototype.getClosestConfig = function(path){
    ...
    for (let k of configKeys ){
        if(k === MAIN)
            k = MAIN_CONFIG_PATH;
        const p = posixPathModule.relative(k, path).slice(posixPathModule.sep);
        if(p.length < length){
            shortest = k;
            length = p.length;
        }
...
Store.prototype.current = async function (name){
    ...
    if(!this.config[key][name].current){
        ...
        let path = posixPathModule.join(posixPathModule.resolve(configPath), this.config[key][name].path);
        while(true){
            let list;
            try{
        ...
        path = posixPathModule.relative( posixPathModule.join(key, this.config[key][name].path), path);
...
Store.prototype.next = async function (name){
    ...
    while(currentPath.length){
        ...
        if(currentIndex < files.length -1){
            ...
            if((await fs.stat(posixPathModule.join(key, this.config[key][name].path,p))).isDirectory()){
                folder = true;
